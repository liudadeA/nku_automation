...
速度指令发布策略:
  - 订阅状态机 /control_state, 仅在 state=2(视觉伺服) 或 state=3(机械臂伸出) 时发布速度指令
...
class VisualApproach:
    def __init__(self):
        ...
        # ── 订阅 ──
        self.state_sub = rospy.Subscriber('/control_state', Int32, self.state_callback)
        ...
        self.record_pub = rospy.Publisher('/approach_cmd_record', Twist, queue_size=1000)

        # ── 目标检测框 ──
        self.target_xmin = 259
        self.target_ymin = 114
        self.target_xmax = 374
        self.target_ymax = 471
        self.target_center_x = 316.5
        self.target_height   = 357    

        # ── 控制参数 ──
        self.angular_speed = rospy.get_param('~angular_speed', 0.3)
        self.linear_speed  = rospy.get_param('~linear_speed', 0.15)
        self.center_threshold = 0.01
        self.fine_tune_threshold = rospy.get_param('~fine_tune_threshold', 0.20)  # ±20%切微调
        ...
        self.micro_speed = rospy.get_param('~micro_speed', 0.04)
        ...
        rospy.loginfo("  Target: center_x=%.1f height=%d (%d,%d -> %d,%d)",
                      self.target_center_x, self.target_height,
                      self.target_xmin, self.target_ymin,
                      self.target_xmax, self.target_ymax)
        ...
        with self.lock:
            ...
            if self.machine_state in ACTIVE_STATES and prev not in ACTIVE_STATES:
                if self.control_phase == -1:
                    ...
                    self.in_fine_tune = False
                    self.stop_robot()
                    rospy.loginfo("状态机→%d, 开始视觉伺服靠近", self.machine_state)
    ...
    def _publish_cmd(self, cmd):
        """发布速度指令, 同时录制到 /approach_cmd_record"""
        self.cmd_pub.publish(cmd)
        if self.machine_state in ACTIVE_STATES:
            self.record_pub.publish(cmd)
    ...
    def check_angular_stable(self):
        ...
        if deviation < self.center_threshold:
            ...
            elif time.time() - self.angular_stable_start >= 0.5:
                return True
        else:
            self.angular_stable_start = None
        return False
    ...
    def check_linear_stable(self):
        ...
        if lower <= self.bottle_height <= upper:
            ...
            elif time.time() - self.linear_stable_start >= 0.5:
                return True
        else:
            self.linear_stable_start = None
        return False
    ...
    def run(self):
        while not rospy.is_shutdown():
            with self.lock:
                # 仅在状态机活跃状态时执行视觉伺服逻辑
                if self.machine_state in ACTIVE_STATES and self.bottle_detected:
                    ...
                    if self.control_phase == 0:
                        ...
                        self._publish_cmd(self.smooth_cmd(cmd))
