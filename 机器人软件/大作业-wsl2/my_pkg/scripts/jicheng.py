...
class ArmController:
    def __init__(self):
        ...
        # 初始化：等待控制器就绪，再移动到位姿1
        rospy.loginfo("等待 Dynamixel 控制器就绪 (2s)...")
        rospy.sleep(2.0)
    ...
    def move_to_pose(self, pose):
        """从 prev_pose_index 平滑插值移动到目标位姿"""
        current_angles = self.poses[self.prev_pose_index]  # 用保存的旧位姿
        delta_angles = [target - current for target, current in zip(pose, current_angles)]
        steps = max(1, int(abs(max(delta_angles, key=abs)) * 10))  # 动态计算步数
        ...
        for step in range(steps + 1):
            if rospy.is_shutdown():
                break
            t = float(step) / steps
            current_step = [current + delta * t for current, delta in zip(current_angles, delta_angles)]
