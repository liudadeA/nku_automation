...
def detector(image_path, models, device=None, detect_mode=1):
    try:
        ...
        # 根据detect_mode决定使用哪些模型和类别
        if detect_mode == 0:  # 只检测人
            detect_classes = [0]
            using_models = {k: v for k, v in models.items() if 'traffic' not in k}
        elif detect_mode == 1:  # 检测所有目标
            detect_classes = None  # None表示检测所有类别
            using_models = models  # 使用所有模型
        
        # 使用选定的模型进行检测
        all_detections = []
        for model_name, model in using_models.items():
            try:
        ...
        # 在返回结果前添加验证
        if results["detection_info"]["total_detections"] == 0:
            logging.warning(f"警告：未检测到任何目标 - {image_path}")
            print(f"警告：未检测到任何目标")
...
if __name__ == "__main__":
    ...
    # 加载通用模型和交通场景专用模型
    model_names = {
        "general": ["yolo12x.pt", "yolo11x-seg.pt"],
        "traffic": ["yolov8x-traffic.pt"]  # 确保文件存在
    }
    
    models = {}
    try:
        # 加载通用模型
        for name in model_names["general"]:
            models[name] = YOLO(name)
        # 加载交通场景模型
        for name in model_names["traffic"]:
            models[name] = YOLO(name)
    except Exception as e:
