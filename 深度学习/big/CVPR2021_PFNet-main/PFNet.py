...
import backbone.pvt_v2 as pvt_v2
...
class PFNet(nn.Module):
    def __init__(self, backbone_path=None):
        ...
        # backbone
        self.backbone = pvt_v2.pvt_v2_b2(backbone_path)

        # channel reduction
        self.cr4 = nn.Sequential(nn.Conv2d(512, 512, 3, 1, 1), nn.BatchNorm2d(512), nn.ReLU())
        self.cr3 = nn.Sequential(nn.Conv2d(320, 256, 3, 1, 1), nn.BatchNorm2d(256), nn.ReLU())
        self.cr2 = nn.Sequential(nn.Conv2d(128, 128, 3, 1, 1), nn.BatchNorm2d(128), nn.ReLU())
        self.cr1 = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
        ...
        self.edge_conv1 = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
        self.edge_conv2 = nn.Sequential(nn.Conv2d(128, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
        self.edge_conv3 = nn.Sequential(nn.Conv2d(320, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
        self.edge_conv4 = nn.Sequential(nn.Conv2d(512, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
        self.edge_fusion = nn.Sequential(nn.Conv2d(256, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU(),
    ...
    def forward(self, x):
        # x: [batch_size, channel=4, h, w] (RGB + depth)
        # backbone forward
        ...
        # edge detection
        edge1 = self.edge_conv1(layer1)
        edge2 = self.edge_conv2(layer2)
        edge3 = self.edge_conv3(layer3)
        edge4 = self.edge_conv4(layer4)
        
        # upsampling to the same size
        edge2 = F.interpolate(edge2, size=edge1.size()[2:], mode='bilinear', align_corners=True)
        edge3 = F.interpolate(edge3, size=edge1.size()[2:], mode='bilinear', align_corners=True)
        edge4 = F.interpolate(edge4, size=edge1.size()[2:], mode='bilinear', align_corners=True)
        
        # edge fusion
        edge_fusion = torch.cat((edge1, edge2, edge3, edge4), dim=1)
        edge_output = self.edge_fusion(edge_fusion)
        edge_output = F.interpolate(edge_output, size=x.size()[2:], mode='bilinear', align_corners=True)
        ...
        if self.training:
            return predict4, predict3, predict2, predict1, edge_output

        return torch.sigmoid(predict4), torch.sigmoid(predict3), torch.sigmoid(predict2), torch.sigmoid(predict1), torch.sigmoid(edge_output)
