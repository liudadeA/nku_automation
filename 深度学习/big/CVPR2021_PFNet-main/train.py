...
img_transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406, 0.5], [0.229, 0.224, 0.225, 0.5])  # RGB-D 4-channel normalization
])
...
def train(net, optimizer):
    ...
    for epoch in range(args['last_epoch'] + 1, args['last_epoch'] + 1 + args['epoch_num']):
        loss_record, loss_1_record, loss_2_record, loss_3_record, loss_4_record, loss_edge_record = AvgMeter(), AvgMeter(), AvgMeter(), AvgMeter(), AvgMeter(), AvgMeter()

        # 使用tqdm创建进度条，设置leave=True保留进度条，dynamic_ncols自动调整宽度
        ...
        for data in train_iterator:
            ...
            # Get 4 saliency predictions and 1 edge output
            predict_1, predict_2, predict_3, predict_4, edge_output = net(inputs)
            ...
            if curr_iter % 10 == 0:
                ...
                writer.add_scalar('loss_edge', loss_edge, curr_iter)

            # Update log file
            log = '[%3d], [%6d], [%.6f], [%.5f], [%.5f], [%.5f], [%.5f], [%.5f], [%.5f]' % \
                  (epoch, curr_iter, base_lr, loss_record.avg, loss_1_record.avg, loss_2_record.avg,
                   loss_3_record.avg, loss_4_record.avg, loss_edge_record.avg)
            # 使用set_postfix显示loss信息，而不是覆盖整个描述
            train_iterator.set_postfix(loss=loss_record.avg.item(), lr=base_lr)
